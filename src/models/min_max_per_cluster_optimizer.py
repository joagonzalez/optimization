...
class MinMaxPerClusterOptimizer(BaseVMOptimizer):
    ...
    def create_model(self):
        ...
        self.z = self.mdl.continuous_var_dict(
            ((c, r) for c in self.clusters for r in self.resources),
            name='z'
        )
    ...
    def solve(self):
        ...
        cluster_utilization = {c: {r: 0.0 for r in self.resources}
                             for c in self.clusters}
        ...
        final_utilization = max(
            ...
            for utilization in cluster_utilization.values()
        )
