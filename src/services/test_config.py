...
from src.models.baseline_optimizer_inv import BaselineOptimizerInv  # noqa
...
def generate_test_scenarios():
    ...
    # Scenario 6: Unbalanced initial with baseline optimizer inv
    scenarios.append(
        TestConfig(
            name="unbalanced_initial_baseline_inv",
            num_vms=100,
            clusters=["c1", "c2", "c3"],
            cluster_capacity={
                "c1": {"cpu": 100.0, "mem": 100.0, "disk": 100.0},
            ...
            vm_demand_ranges={"cpu": (0.01, 1), "mem": (0.05, 2), "disk": (0.1, 3)},
            optimizer_model=BaselineOptimizerInv,
        )
    )

    # Scenario 7: Unbalanced initial with min-max per cluster optimizer
    scenarios.append(
        TestConfig(
